...
bool settings_load(settings_t* s)
{
    ...
    s->hops = 3;
    nvs_handle_t nvs;
    esp_err_t err = nvs_open("meshbbs_mc", NVS_READONLY, &nvs);
...
{
    nvs_handle_t nvs;
    if (nvs_open("meshbbs_mc", NVS_READWRITE, &nvs) != ESP_OK)
        return false;
    esp_err_t err = nvs_set_str(nvs, "target", s->target);
    if (err == ESP_OK)
        err = nvs_set_i32(nvs, "pin", s->pin);
    if (err == ESP_OK)
        err = nvs_set_u8(nvs, "hops", s->hops);
    if (err == ESP_OK)
        err = nvs_commit(nvs);
    nvs_close(nvs);
    return err == ESP_OK;
