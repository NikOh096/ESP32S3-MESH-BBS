...
void diagnostics_init(void)
{
    ...
    nvs_handle_t handle;
    if (nvs_open_from_partition("bbs", "mc_diagnostics", NVS_READONLY, &handle) == ESP_OK) {
...
void diagnostics_flush(void)
{
    if (!dirty)
        return;
    nvs_handle_t handle;
    esp_err_t rc = nvs_open_from_partition("bbs", "mc_diagnostics", NVS_READWRITE, &handle);
    if (rc == ESP_OK) {
...
size_t diagnostics_report(char* out, size_t capacity)
{
    if (capacity < 80)
        return 0;
    int n = last_node ? snprintf(out, capacity, "MeshCore BBS alias !%08" PRIx32 "; DM the companion contact in MeshCore.\n", last_node) : snprintf(out, capacity, "Node ID not captured yet; save settings and start BBS.\n");
    if (!activity.count)
