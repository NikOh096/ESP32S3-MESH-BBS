...
/* Parse upstream BluetoothCommon.h UUIDs rather than hand-transcribing their
 * reversed byte arrays; a typo can find the service but miss its message I/O. */
static ble_uuid_any_t other_service_uuid;
static ble_uuid_any_t service_uuid, to_uuid, from_uuid, num_uuid;
...
static uint16_t svc_start, svc_end, to_handle, from_handle, num_handle, num_end, cccd;
static uint8_t own_type;
static bool synced, connecting, ready, busy, drain;
static _Atomic bool force_reconnect;
static _Atomic status_led_mode_t link_status = STATUS_WAITING;
static int64_t next_scan, last_read, op_started;
static link_event_t read_event;
...
static void inspect_advertisement(const struct ble_gap_disc_desc* disc)
{
    ...
    bool selectable = selected && selected->protocol == 1;
    if (selected && !strcasecmp(cfg.target, selected->name))
        match = true;
    xSemaphoreGive(scan_mutex);
    if ((mesh_service || !strncmp(name, "Meshtastic", 10) || match) && (strcmp(address, last_logged) || esp_timer_get_time() - last_logged_at > 10000000)) {
    ...
    struct ble_gatt_attr* attr, void* arg)
{
    ...
    pair_deadline = 0;
    ESP_LOGI(TAG, "Meshtastic BLE service ready; MTU=%u", ble_att_mtu(conn));
    emit(LINK_UP);
    return 0;
}
    ...
    const struct ble_gatt_chr* chr, void* arg)
{
    ...
    (void)arg;
    if (!error->status) {
        if (num_handle && chr->def_handle > num_handle && chr->def_handle - 1 < num_end)
            num_end = chr->def_handle - 1;
        ...
        if (!ble_uuid_cmp(&chr->uuid.u, &num_uuid.u)) {
            num_handle = chr->val_handle;
            num_end = svc_end;
        }
    } else if (error->status == BLE_HS_EDONE) {
        ESP_LOGI(TAG, "Message handles ToRadio=%u FromRadio=%u FromNum=%u",
            to_handle, from_handle, num_handle);
        if (!to_handle || !from_handle || !num_handle) {
            fail("required characteristic missing", -1);
            return 0;
        }
        int rc = ble_gattc_disc_all_dscs(conn, num_handle, num_end, descriptor_found, NULL);
        if (rc)
    ...
    const struct ble_gatt_svc* svc, void* arg)
{
    ...
    if (!error->status) {
        ...
        if (!svc_start) {
            fail("Meshtastic service missing", -1);
            return 0;
        }
    ...
    uint16_t mtu, void* arg)
{
    ...
    (void)arg;
    if (error->status)
        ESP_LOGW(TAG, "MTU negotiation status %d; long GATT operations available", error->status);
    else
...
static int read_complete(uint16_t handle, const struct ble_gatt_error* error,
    struct ble_gatt_attr* attr, void* arg)
{
    (void)handle;
    (void)arg;
    if (!error->status && attr) {
        ...
        if (attr->offset != read_event.length || read_event.length + len > sizeof(read_event.bytes)) {
            fail("invalid/oversize GATT read", -1);
            return BLE_HS_EAPP;
        }
        if (os_mbuf_copydata(attr->om, 0, (int)len, read_event.bytes + read_event.length)) {
            fail("GATT mbuf copy", -1);
            return BLE_HS_EAPP;
        }
        read_event.length += len;
    } else if (error->status == BLE_HS_EDONE) {
        busy = false;
        last_read = esp_timer_get_time();
        if (read_event.length) {
            read_event.kind = LINK_DATA;
            if (xQueueSend(rxq, &read_event, 0) != pdTRUE) {
                fail("receive queue overflow", -1);
                return 0;
            }
            drain = true;
        }
        ...
        busy = false;
        fail("FromRadio read", error->status);
    }
    return 0;
}
...
static void pump(struct ble_npl_event* event)
{
    ...
    if (xQueueReceive(txq, &tx, 0) == pdTRUE) {
        ...
        if (tx.length <= ble_att_mtu(conn) - 3) {
            ...
            struct os_mbuf* mb = ble_hs_mbuf_from_flat(tx.bytes, tx.length);
            if (!mb)
                rc = BLE_HS_ENOMEM;
            else
                rc = ble_gattc_write_long(conn, to_handle, 0, mb, write_complete, NULL);
        }
        if (rc) {
            busy = false;
            emit(LINK_TX_FAILED);
    ...
    } else if ((drain || now - last_read >= 2000000) && uxQueueSpacesAvailable(rxq) > 1) {
        drain = false;
        busy = true;
        op_started = now;
        read_event.length = 0;
        int rc = ble_gattc_read_long(conn, from_handle, 0, read_complete, NULL);
        if (rc) {
            busy = false;
            fail("FromRadio read start", rc);
        }
    }
}
...
static int gap_cb(struct ble_gap_event* event, void* arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        atomic_store(&link_status, STATUS_CONNECTING);
        svc_start = svc_end = to_handle = from_handle = num_handle = num_end = cccd = 0;
    ...
    case BLE_GAP_EVENT_NOTIFY_RX:
        if (event->notify_rx.attr_handle == num_handle)
            drain = true;
        break;
...
{
    cfg = *settings;
    ESP_ERROR_CHECK(ble_uuid_from_str(&other_service_uuid, "6e400001-b5a3-f393-e0a9-e50e24dcca9e"));
    ESP_ERROR_CHECK(ble_uuid_from_str(&service_uuid, "6ba1b218-15a8-461f-9fa8-5dcae273eafd"));
    ESP_ERROR_CHECK(ble_uuid_from_str(&to_uuid, "f75c76d2-129e-4dad-a1dd-7866124401e7"));
    ESP_ERROR_CHECK(ble_uuid_from_str(&from_uuid, "2c55e69e-4993-11ed-b878-0242ac120002"));
    ESP_ERROR_CHECK(ble_uuid_from_str(&num_uuid, "ed9da18c-a800-4f66-a670-aa7547e34453"));
